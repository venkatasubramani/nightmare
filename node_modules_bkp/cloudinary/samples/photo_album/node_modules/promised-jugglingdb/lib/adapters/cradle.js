...
function filtering(res, model, filter, instance){
  ...
  if (filter.order) {
    ...
    if (allNumeric) {
      res = res.sort(curry(numerically,orders));
    } else {
      res = res.sort(curry(literally,orders));
    }
    if (reverse) {
      res = res.reverse();
    }
...
CradleAdapter.prototype.models = function (model, filter, callback, func){
  ...
  self.client.save('_design/' + table, {
    ...
    self.client.view(table + '/all', {include_docs: true, limit: limit, skip: skip}, errorHandler(callback, curry(function (res, cb){
...
CradleAdapter.prototype.destroyAll = function (model, callback){
  this.models(
    ...
    null,
    callback,
    curry(function (docs, cb){
      var docIds = docs.map(function (doc){
        return doc.id;
      });
      this.client.get(docIds, curry(function (err, res){
        if (err) {
          cb(err);
        }

        var funcs = res.map(curry(function (doc){
          return curry(this.client.remove, this.client);
        }, this));
