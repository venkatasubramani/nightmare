...
Neo4j.prototype.node = function find(id, callback){
  if (this.cache[id]) {
    ...
    this.client.getNodeById(id, curry(function (err, node){
      if (node) {
        this.cache[id] = node;
      }
      callback(err, node);
...
Neo4j.prototype.create = function create(model, data, callback){
  ...
  node.save(curry(function (err){
    if (err) {
      return callback(err);
    }
    this.cache[node.id] = node;
    node.index(table, 'id', node.id, curry(function (err){
      if (err) {
        return callback(err);
      }
...
Neo4j.prototype.find = function find(model, id, callback){
  delete this.cache[id];
  this.node(id, curry(function (err, node){
    if (node && node.data) {
      node.data.id = id;
    }
...
Neo4j.prototype.destroy = function destroy(model, id, callback){
  ...
  this.node(id, function (err, node){
    ...
    node.delete(curry(function (err){
      if (err) {
        return callback(err);
      }
      delete this.cache[id];
...
Neo4j.prototype.all = function all(model, filter, callback){
  this.client.queryNodeIndex(this.table(model), 'id:*', curry(function (err, nodes){
    if (nodes) {
      nodes = nodes.map(curry(function (obj){
        obj.data.id = obj.id;
